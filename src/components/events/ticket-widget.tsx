...
import {
  Calendar, Check, ChevronDown, Clock,
  Minus, Plus, Tag, Ticket, X, ShieldCheck,
} from "lucide-react";
...
function useTicketState({ basePrice, schedule }: Pick<TicketSelectorProps, "basePrice" | "schedule">) {
  ...
  return {
    isMultiDay, schedule, selectedDay, dayDropOpen, setDayDropOpen,
    quantities, timeLeft, promoOpen, setPromoOpen, promoInput, setPromoInput,
    appliedPromo, setAppliedPromo, promoError, setPromoError,
    tiers, updateQty, pickDay, applyPromo,
    totalQty, subtotal, discount, total, lowestPrice,
  };
}

/* ─── Shared ticket body ─────────────────────────────────── */
function TicketBody({
  slug, state, compact = false,
}: {
  slug: string;
  state: ReturnType<typeof useTicketState>;
  compact?: boolean;
}) {
  const {
    isMultiDay, schedule, selectedDay, dayDropOpen, setDayDropOpen,
    ...
    appliedPromo, setAppliedPromo, promoError, setPromoError,
    tiers, updateQty, pickDay, applyPromo,
    totalQty, subtotal, discount, total,
  } = state;
  ...
  return (
    <>
      {/* Countdown */}
      {timeLeft > 0 && !compact && (
        <div className={cn("border-b border-border px-5 py-4", !compact && "rounded-t-2xl overflow-hidden")}>
      ...
      <div className={compact ? "" : "p-5"}>
        ...
        {isMultiDay && schedule && (
          <div className="mb-5">
            ...
            {/* Mobile sheet → inline list (no absolute, no clipping) */}
            {compact ? (
              <div className="flex flex-col divide-y divide-border overflow-hidden rounded-xl border border-border">
                {schedule.map((d, i) => (
                  <button key={i} type="button"
                    onClick={() => pickDay(i)}
                    className={cn("flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition-colors", selectedDay === i ? "bg-muted" : "hover:bg-muted/50")}
                  >
                    <span className={cn("flex size-6 shrink-0 items-center justify-center rounded-full border text-[10px] font-semibold", selectedDay === i ? "border-foreground bg-foreground text-background" : "border-border text-muted-foreground")}>{i + 1}</span>
                    <span className="flex flex-col leading-snug">
                      <span className="font-medium text-foreground">{d.day}</span>
                      <span className="text-xs text-muted-foreground">{d.date}</span>
                    </span>
                    {selectedDay === i && <Check className="ml-auto size-4 shrink-0" />}
                  </button>
                ))}
                <button type="button"
                  onClick={() => pickDay(-1)}
                  className={cn("flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition-colors", selectedDay === -1 ? "bg-muted" : "hover:bg-muted/50")}
                >
                  <span className={cn("flex size-6 shrink-0 items-center justify-center rounded-full border text-[10px]", selectedDay === -1 ? "border-foreground bg-foreground text-background" : "border-border text-muted-foreground")}>✦</span>
                  <span className="flex flex-col leading-snug">
                    <span className="flex items-center gap-1.5 font-medium text-foreground">
                      บัตรทุกวัน ({schedule.length} วัน)
                      <span className="rounded-full border border-border px-1.5 py-px text-[9px] text-muted-foreground">ประหยัดสุด</span>
                    </span>
                    <span className="text-xs text-muted-foreground">{schedule[0].date} — {schedule[schedule.length - 1].date}</span>
                  </span>
                  {selectedDay === -1 && <Check className="ml-auto size-4 shrink-0" />}
                </button>
              </div>
            ) : (
                    ...
                    <div className="absolute left-0 right-0 top-full z-40 mt-1 overflow-hidden rounded-xl border border-border bg-card shadow-lg">
                      {schedule.map((d, i) => (
                        <button key={i} type="button"
                          ...
                          className={cn("flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition-colors hover:bg-muted", selectedDay === i && "bg-muted")}
                        >
                          <span className={cn("flex size-5 shrink-0 items-center justify-center rounded-full border text-[10px] font-semibold", selectedDay === i ? "border-foreground bg-foreground text-background" : "border-border text-muted-foreground")}>{i + 1}</span>
                          <span className="flex flex-col leading-snug">
                            <span className="font-medium text-foreground">{d.day}</span>
                            <span className="text-xs text-muted-foreground">{d.date}</span>
                          </span>
                      ...
                      <button type="button"
                        ...
                        className={cn("flex w-full items-center gap-3 px-4 py-3 text-left text-sm transition-colors hover:bg-muted", selectedDay === -1 && "bg-muted")}
                      >
          ...
          <>
              ...
              {appliedPromo ? (
                ...
                <div className="rounded-xl border border-border p-3.5">
                  ...
                  <button type="button" onClick={() => { setPromoOpen(false); setPromoError(""); }}
                    className="mt-2 text-xs text-muted-foreground hover:text-foreground transition-colors">ยกเลิก</button>
                </div>
              ) : (
                <button type="button" onClick={() => setPromoOpen(true)}
                  className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
                  <Tag className="size-3" />มีรหัสโปรโมชั่น?
                </button>
              )}
            </div>
              ...
              {discount > 0 && (
                <>
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span>ราคารวม</span><span>฿{subtotal.toLocaleString("th-TH")}</span>
                  </div>
            ...
            <button type="button" disabled={totalQty === 0}
              onClick={() => {
                ...
                const qtyEntries = Object.entries(quantities).filter(([, v]) => v > 0);
                if (qtyEntries.length) p.set("qty", qtyEntries.map(([k, v]) => `${k}:${v}`).join(","));
                if (appliedPromo) p.set("promo", appliedPromo.code);
                window.location.href = `/checkout/${slug}?${p.toString()}`;
              }}
              className={cn(
                "mt-4 flex h-12 w-full items-center justify-center gap-2 rounded-xl text-sm font-semibold transition-all",
                totalQty > 0 ? "bg-foreground text-background hover:opacity-90" : "cursor-not-allowed bg-muted text-muted-foreground"
              )}>
...
/* ─── Main export ────────────────────────────────────────── */
export function TicketWidget({ slug, basePrice, schedule }: TicketSelectorProps) {
  const state = useTicketState({ basePrice, schedule });
  ...
  const { totalQty, total, lowestPrice, selectedDay, quantities, appliedPromo } = state;
    ...
    <>
      ...
      <div className="hidden lg:block">
        <div className="rounded-2xl border border-border bg-card">
          <TicketBody slug={slug} state={state} />
        </div>
      </div>

      {/* ── Mobile bottom bar (floating card) ───────── */}
      <div className="fixed bottom-0 left-0 right-0 z-50 px-4 pb-5 lg:hidden" style={{ paddingBottom: "max(1.25rem, env(safe-area-inset-bottom))" }}>
        <div className="flex items-center gap-3 rounded-2xl border border-border bg-card/95 px-4 py-3 shadow-xl shadow-black/10 backdrop-blur-md">
          <div className="flex-1 min-w-0">
            {totalQty > 0 ? (
              <div className="flex items-baseline gap-2">
                <p className="text-lg font-bold text-foreground">฿{total.toLocaleString("th-TH")}</p>
                <p className="text-xs text-muted-foreground">{totalQty} ใบ</p>
              </div>
            ) : (
              <div>
                <p className="text-[11px] text-muted-foreground">ราคาเริ่มต้น</p>
                <p className="text-lg font-bold text-foreground">
                  {lowestPrice === 0 ? "ฟรี" : `฿${lowestPrice.toLocaleString("th-TH")}`}
                </p>
              </div>
            )}
          </div>
          <button
            type="button"
            onClick={() => totalQty > 0 ? goToCheckout() : setSheetOpen(true)}
            className="flex h-11 shrink-0 items-center gap-2 rounded-xl bg-foreground px-5 text-sm font-semibold text-background transition-opacity active:opacity-80"
          >
            <Ticket className="size-4" />
            {totalQty > 0 ? "ดำเนินการต่อ" : "เลือกบัตร"}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {sheetOpen && (
          <div className="fixed inset-0 z-[60] lg:hidden">
            ...
            {/* sheet */}
            <SlideUp
              className="absolute bottom-0 left-0 right-0 flex max-h-[92dvh] flex-col overflow-hidden rounded-t-2xl border-t border-border bg-card"
            >
              {/* drag handle + header */}
              <div className="flex shrink-0 flex-col items-center border-b border-border px-5 pb-3 pt-3">
                ...
                <div className="flex w-full items-center justify-between">
                  <h3 className="font-semibold text-foreground">เลือกบัตร</h3>
                  <button
                    type="button"
                    onClick={() => setSheetOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto p-5 pb-[env(safe-area-inset-bottom)]">
                <TicketBody slug={slug} state={state} compact />
              </div>
            </SlideUp>
          </div>
        )}
